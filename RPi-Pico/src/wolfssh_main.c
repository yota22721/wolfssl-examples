...
"-----END RSA PRIVATE KEY-----";
    ...
    #define ECC_PATH server_key_ecc
#endif
...
static int load_key(byte isEcc, byte* buf, word32 bufSz)
{
    ...
    bufName = isEcc ? ECC_PATH : server_key_rsa ;
    DerBuffer* derObj = NULL;
    word32 tmpInBufSz = 0;
    if(isEcc) {
        tmpInBufSz = wc_PemToDer(bufName, XSTRLEN(ECC_PATH)+1, PRIVATEKEY_TYPE, &derObj, NULL, NULL, NULL);
    }
    else {
        tmpInBufSz = wc_PemToDer(bufName, XSTRLEN(server_key_rsa)+1, PRIVATEKEY_TYPE, &derObj, NULL, NULL, NULL);
    }
    
    //printf("%s\n",derObj->buffer);
    //sz = load_file(bufName, buf, &bufSz);
    WMEMCPY(buf, derObj->buffer, derObj->length);
    sz = derObj->length;
    //&bufSz = sizeof(bufName);
...
static int SetDefaultSftpPath(WOLFSSH* ssh, const char* defaultSftpPath)
{
    ...
    #else
        if (WGETCWD(ssh->fs, path, sizeof(path)-1) == NULL) {
            ret = WS_INVALID_PATH_E;
            printf("error 1 %d\n",ret);
        }
    #endif
...
THREAD_RETURN WOLFSSH_THREAD echoserver_test(void* args)
{
    ...
    do {
        ...
        wolfSSH_SetFilesystemHandle(ssh, &fs);
    #ifdef WOLFSSH_STATIC_MEMORY
    ...
    #ifdef WOLFSSH_SFTP
        int t;
        if (t = SetDefaultSftpPath(ssh, defaultSftpPath) != 0) {
            printf("error :%d\n",t);
...
//extern BYTE ramdisk_buffer[RAMDISK_SIZE];
void wolfSSH_Echoserver(void* args)
{
    ...
    args.argv = argv;
        ...
        #if defined(USE_RAM)
        BYTE work[FF_MIN_SS]; 
        if (fres=f_mkfs("0:", &parm, work, sizeof(work)) != FR_OK)
        {
...
//#ifndef NO_MAIN_DRIVER
    ...
    int main(void)
    {
        ...
        WMEMSET(&args, 0, sizeof(args));
        #ifndef WOLFSSH_SFTP
        char * cmd[2] = {"./echoServer","-f"};
        #else
        char * cmd[1] = {"./echoServer"};
        #endif
        args.argc = 2;//argc
        args.argv = cmd;
        

        TaskHandle_t task_echoServer;
        //StaticTask_t task_echoServer;
        ...
        printf("Creating echoServer task, stack = %d\n", STACK_SIZE);
        
        xTaskCreate(wolfSSH_Echoserver, "wolfssh_MainThread", STACK_SIZE, &args,
                    CYW43_TASK_PRIORITY + 1, &task_echoServer);
        vTaskStartScheduler();
        #endif
